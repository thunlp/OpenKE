...
class Config(object):
	def __init__(self):
		...
		''''valid'''
		self.lib.getValidHeadBatch.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p]
		self.lib.getValidTailBatch.argtypes = [ctypes.c_void_p, ctypes.c_void_p, ctypes.c_void_p]
	...
	def set_train_model(self, model):
		...
		if self.optimizer != None:
			...
			self.optimizer = optim.Adadelta(self.trainModel.parameters(), lr = self.alpha, weight_decay = self.weight_decay)
		elif self.opt_method == "Adam" or self.opt_method == "adam":
			self.optimizer = optim.Adam(self.trainModel.parameters(), lr = self.alpha, weight_decay = self.weight_decay)
		else:
			self.optimizer = optim.SGD(self.trainModel.parameters(), lr = self.alpha, weight_decay = self.weight_decay)
	...
	def save_checkpoint(self, model, epoch):
		path = os.path.join(self.checkpoint_dir, self.model.__name__ + '-' + str(epoch) + '.ckpt')
	...
	def link_prediction(self):
		...
		for i in range(self.testTotal):
			...
			res = self.test_one_step(self.testModel, self.test_h, self.test_t, self.test_r)
	...
	def triple_classification(self):
		...
		res_pos = self.test_one_step(self.testModel, self.valid_pos_h, self.valid_pos_t, self.valid_pos_r)
		res_neg = self.test_one_step(self.testModel, self.valid_neg_h, self.valid_neg_t, self.valid_neg_r)
		...
		res_pos = self.test_one_step(self.testModel, self.test_pos_h, self.test_pos_t, self.test_pos_r)
		res_neg = self.test_one_step(self.testModel, self.test_neg_h, self.test_neg_t, self.test_neg_r)
