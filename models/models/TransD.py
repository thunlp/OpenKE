...
class TransD(Model):
	...
	def embedding_def(self):
		...
		self.ent_transfer = tf.keras.layers.Embedding(config.entTotal, config.hidden_size, name="ent_transfer")
		self.rel_transfer = tf.keras.layers.Embedding(config.relTotal, config.hidden_size, name="rel_transfer")
	...
	def transfer_look_up_htr(self, h, t, r):
		return [self.ent_transfer(h), self.ent_transfer(t), self.rel_transfer(r)]
	...
	def loss_def(self):
		...
		pos_h_t, pos_t_t, pos_r_t = self.transfer_look_up_htr(pos_h, pos_t, pos_r)
		neg_h_t, neg_t_t, neg_r_t = self.transfer_look_up_htr(neg_h, neg_t, neg_r)
	...
	def predict_def(self):
		...
		predict_h_e, predict_t_e, predict_r_e = self.embeddings_look_up_htr(predict_h, predict_t, predict_r)
		predict_h_t, predict_t_t, predict_r_t = self.transfer_look_up_htr(predict_h, predict_t, predict_r)
