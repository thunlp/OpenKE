...
class ComplEx(Model):
	...
	def embedding_def(self):
		...
		self.ent1_embeddings = tf.keras.layers.Embedding(config.entTotal, config.hidden_size, name = "ent_re_embeddings")
		self.rel1_embeddings = tf.keras.layers.Embedding(config.relTotal, config.hidden_size, name = "rel_re_embeddings")
		self.ent2_embeddings = tf.keras.layers.Embedding(config.entTotal, config.hidden_size, name = "ent_im_embeddings")
		self.rel2_embeddings = tf.keras.layers.Embedding(config.relTotal, config.hidden_size, name = "rel_im_embeddings")
	...
	def loss_def(self):
		...
		p1_h = self.ent1_embeddings(pos_h)
		p2_h = self.ent2_embeddings(pos_h)
		p1_t = self.ent1_embeddings(pos_t)
		p2_t = self.ent2_embeddings(pos_t)
		p1_r = self.rel1_embeddings(pos_r)
		p2_r = self.rel2_embeddings(pos_r)
		n1_h = self.ent1_embeddings(neg_h)
		n2_h = self.ent2_embeddings(neg_h)
		n1_t = self.ent1_embeddings(neg_t)
		n2_t = self.ent2_embeddings(neg_t)
		n1_r = self.rel1_embeddings(neg_r)
		n2_r = self.rel2_embeddings(neg_r)
