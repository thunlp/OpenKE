...
class HolE(Model):
	...
	def look_up_htr(self, h, t, r, hidden_size):
		return [tf.reshape(self.ent_embeddings(h), [-1, hidden_size]), tf.reshape(self.ent_embeddings(t), [-1, hidden_size]), tf.reshape(self.rel_embeddings(r), [-1, hidden_size])]
	...
	def loss_def(self):
		...
		pos_h_e, pos_t_e, pos_r_e = self.look_up_htr(pos_h, pos_t, pos_r, config.hidden_size)
		neg_h_e, neg_t_e, neg_r_e = self.look_up_htr(neg_h, neg_t, neg_r, config.hidden_size)
	...
	def predict_def(self):
		...
		predict_r_e = self.rel_embeddings(predict_r)
		self.predict = tf.reduce_mean(self._calc(predict_h_e, predict_t_e, predict_r_e), 1, keep_dims = True)
