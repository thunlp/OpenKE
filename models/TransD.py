...
class TransD(Model):
	...
	def loss_def(self):
		...
		p_score =  tf.reduce_sum(tf.reduce_mean(_p_score, axis = -2, keepdims = False), axis = -1, keepdims = True)
		n_score =  tf.reduce_sum(tf.reduce_mean(_n_score, axis = -2, keepdims = False), axis = -1, keepdims = True)
